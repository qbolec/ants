    ...
    this.listenTo(this.model,'change:players',(model,players)=>{
      _.each(['player1','player2'],(classname)=>{
        ...
        _.each(players,(player)=>{
          ...
          option.attr('value',player.source?'s'+player.source:'a'+player.ast);
...
class ArenaPage extends Page {
  ...
  getAstFromControl(selector){
    var text = (<HTMLInputElement>this.view.$(selector)[0]).value;
    if(text[0]=='a'){
      return JSON.parse(text.substr(1));
    }else{
      var source=text.substr(1);
      try{
        return (new TaskTemplateExecutor()).parse(source);
      }catch(e){
        return null;
      }
    }
  ...
  run(){
    ...
    var arena = new Arena(this.getAstFromControl('select.player1'),this.getAstFromControl('select.player2'),terrain);
